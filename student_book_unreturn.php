...
if (Input::exists()) {
	...
	try{
		...
		$user->create_book_unreturn(array(
			'student_id' => Input::get('s_id'),
			'book_id' => $book_id,
			'date_unreturn' => date('m-d-Y')
		));
...
<body>
	...
	<form action = "" method = "post">
		<table>
			...
			if($user->count() == 0){
				echo '<td colspan = "6">No book Un-Return for now </td>';
			}
			foreach($user->results() as $user){ 
			?>	
				<td>
				<input type = "hidden" name = "s_id" value = <?php echo  $user->s_id;?> >
				<input type = "hidden" name = "bb_id" value = <?php echo  $user->bb_id;?> >
			</tr>
			<tr>
		...
		<?php } ?>
			<tr>
				<td><input type= "submit" name = "submit" value = "Un-Return book"></td>
			</tr>
		</table>
	</form>
